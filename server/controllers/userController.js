    ...
    try {
        ...
        if (!process.env.STRIPE_SECRET_KEY) {
            return res.status(500).json({
                success: false,
                message: "Missing STRIPE_SECRET_KEY. Add it to server/.env before purchasing courses."
            });
        }
        ...
        if (!process.env.CURRENCY) {
            return res.status(500).json({
                success: false,
                message: "Missing CURRENCY. Add it to server/.env, for example CURRENCY=usd."
            });
        }
...
// Confirm UPI payment in local setup and enroll user
...
export const confirmUpiPayment = async (req,res) => {
    try {
        ...
        res.json({ success: true, message: alreadyEnrolled ? "Already Enrolled" : "Payment successful. Course enrolled!" });
    } catch (error) {
        res.json({success: false, message:error.message})
    }
}
